#!/usr/bin/env python3
"""
Assemble a report.json for aura_pdf following references/aura_report_schema.md.
"""
...
def build_report(
    *,
    ...
    apply_errors = apply_report.get("errors")
    apply_ok = not (isinstance(apply_errors, list) and len(apply_errors) > 0)
    overall_status = "success" if (ok_a and apply_ok) else "failed_gate_a"
    ...
    # Include any apply-time artifacts (extractions, etc.)
    extra = apply_report.get("artifacts")
    if isinstance(extra, list):
        for a in extra:
            if isinstance(a, dict) and isinstance(a.get("type"), str) and isinstance(a.get("path"), str):
                artifacts.append({"type": a["type"], "path": a["path"]})
    ...
    return {
        ...
        "gates": {
            "gate_a": gate_a,
            "gate_b": {"required": False, "skipped": False, "ok": None, "reason": "N/A for pdf"},
        },
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build aura_pdf report.json")
