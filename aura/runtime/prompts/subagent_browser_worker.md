...
Recommended inside Aura: prefer `agent-browser screenshot --full` (no path; stdout→artifact) to avoid writing files. If you must write a path, it will often require approval; keep it under `artifacts/` and use **project-relative paths only** (no absolute paths).
